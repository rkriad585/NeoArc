...
def login():
    if request.method == 'POST':
        ...
        # Verify against config.py
        if email == config.ADMIN_CREDENTIALS['email'] and password == config.ADMIN_CREDENTIALS['password']:
            session['is_admin'] = True
            return redirect(url_for('admin.dashboard'))
        else:
            flash('Invalid Admin Credentials', 'error')
            
    # Explicitly point to the admin folder
    return render_template('admin/login.html')

@admin_bp.route('/')
def dashboard():
    conn = get_db()
    # Get all users and count their aliases
    users = conn.execute('''
...
def block_user(user_id):
    conn = get_db()
    # Toggle Block Status (0 = Active, 1 = Blocked)
